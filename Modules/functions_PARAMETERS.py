...
def get_physical_units(mode = 'SI'):
    ...
    if mode.lower() == 'si':
        ...
        dic['ratio'] = ['adimensional', '%']
        dic['temperature'] = ['C', 'o C', 'oC', 'K']
        dic['time'] = ['s', 'sec', 'secs', 'Sec', 'Secs', 'min', 'mins', 'Min', 'Mins', 'h', 'hour', 'hours', 'Hour', 'Hours', 'day', 'days', 'Day', 'Days', 'year', 'years', 'Year', 'Years', 'yr', 'yrs']
        ...
        for i in range(len(units['raw_unit'])):
            ...
            if '–' in units['raw_unit'][i]:
                ...
                if inverse_PU not in SI_unis_list:            
                    ...
                    conv_factor_to_multiply = conv_factor_to_multiply * ( 1 / units['factor_list'][i] )
                ...
                if direct_PU not in SI_unis_list:
                    ...
                    if units['factor_operation'][i] == 'add':
                        ...
                        #caso a conversão seja por multiplicação
                        conv_factor_to_multiply = conv_factor_to_multiply * units['factor_list'][i]
                        SI_unis_list.append( direct_PU )
...
def regex_patt_from_parameter(parameter):
    ...
    #esse termo é para procurar parâmetros adimensionais (ex: elemental ratio)
    ...
    if parameter.lower() == 'concentration_mass':
        ...
        #determinando o número mínimo e máximo de caracteres numéricos        
        n_min_len , n_max_len = 2 , 4
        ndec_min_len, ndec_max_len = 1 , 2
        found_parameter = True
    
    elif parameter.lower() == 'percentageweight':
        
        pattern_dic['first_parameter'] = 'percentageweight'
        pattern_dic['second_parameter'] = None
    ...
    elif parameter.lower() == 'ratio_watercontent':
        
        pattern_dic['first_parameter'] = 'ratio'
        pattern_dic['second_parameter'] = None

        #lista de unidades a serem encontradas        
        PU_units_to_find = ['%', 'wtperc']

        #determinando o número mínimo e máximo de caracteres numéricos        
        n_min_len , n_max_len = 2 , 3 
        ndec_min_len, ndec_max_len = 1 , 3
        found_parameter = True
    ...
    pattern_dic['parameter_to_find_in_sent'] = None
    ...
    if found_parameter is True:
        ...
        if find_text_parameter is True:
            ...
            parameters_combination_to_find = ''
            for i in range(len(text_parameter_list)):
                if i == len(text_parameter_list) - 1:
                    parameters_combination_to_find += text_parameter_list[i]
                else:
                    parameters_combination_to_find += text_parameter_list[i] + '|'
            
            #gerando a pattern de texto para encontrar unidades adimensionais
            pattern_dic['parameter_to_find_in_sent'] = parameters_combination_to_find
        ...
        pattern_dic['PU_to_find_regex'] = r'((((?<=[0-9]\s*)({parameters_to_find})?(&|to|or|and|\s)+)|(to|or|and|\(|\;|\,|\s)+)(?<![\>\<]\s)(–?[0-9]{int_min_len},{int_max_len})(\.[0-9]{dec_min_len},{dec_max_len}){decimal_opt}\s*({parameters_to_find})?)+\s*({parameters_to_find})[\s\.\,;:\)\]]'.format(int_min_len = '{' + str(n_min_len),
        ...
        if parameter_type == 'single':
            ...
            pattern_dic['PU_not_to_find_regex'] = r'((((?<=[0-9]\s*)({parameters_not_to_find})?(&|to|or|and|\s)+)|(to|or|and|\(|\;|\,|\s)+)(?<![\>\<]\s)(–?[0-9]{int_min_len},{int_max_len})(\.[0-9]{dec_min_len},{dec_max_len}){decimal_opt}\s*({parameters_not_to_find})?)+\s*({parameters_not_to_find})[\s\.\,;:\)\]]'.format(int_min_len = '{' + str(n_min_len),
                                                                                                                                                                                                                                                                                                                     int_max_len = str(n_max_len) + '}',
                                                                                                                                                                                                                                                                                                                     dec_min_len = '{' + str(ndec_min_len),
                                                                                                                                                                                                                                                                                                                     dec_max_len = str(ndec_max_len) + '}',
    ...
    #print('parameter_to_find_in_sent: ', pattern_dic['parameter_to_find_in_sent'])
    return pattern_dic
