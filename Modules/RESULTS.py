...
class results(object):
    ...
    def plot_1column_network_graph(self, DF_column = '', max_circle_size = 100, min_circle_size = 10, min_occurrences = 0, graph_base_title = '',
                                   graph_type = 'graph', analyze = False, nodes_to_analyze = None, plot = False, print_unique_vals_in_DF = False):
        ...
        self.network_analysis_graph_plot(concat_network_relat, max_circle_size = max_circle_size, min_circle_size = min_circle_size,
                                         graph_type = graph_type, analyze = analyze, nodes_to_analyze=nodes_to_analyze, plot = plot,
    ...
    def plot_multicolumn_network_graph(self, DF_columns = ['0', '1', '2'], max_circle_size = 100, min_circle_size = 10, min_occurrences = 10,
                                       graph_type = 'graph', analyze = False, nodes_to_analyze = None, plot = False, graph_base_title = ''):
    ...
    def network_analysis_graph_plot(self, concat_network_relat, nodes_categories = None, 
                                    ...
                                    graph_type = 'graph', analyze = False, nodes_to_analyze=None, plot = False):
        ...
        if analyze is True:
            ...
            if nodes_to_analyze is None and graph_type.lower() == 'digraph':
                                
                #calculando a centrality dos nodes
                ...
                print('Node with higher in_degree centrality: ', node_higher_degree_cent)
                
                #determinando os in_edges
                in_edges = [edge for edge in G.edges() if edge[1] == node_higher_degree_cent]
                in_edges_G = nx.Graph()
                for source, target in in_edges:
                    in_edges_G.add_edge(source, target, weight=G[source][target]['weight'])
                
                self.network_weighted_plot(in_edges_G, 30, 50, net_layout = 'random', directed_edges = False, 
                                           plot_title = str(graph_base_title) + ' - h. cent. node: ' + str(node_higher_degree_cent) + ' (out)')
                    
                #out_degree                
                #determinando os out_edges
                out_edges = [edge for edge in G.edges() if edge[0] == node_higher_degree_cent]
                out_edges_G = nx.Graph()
                for source, target in out_edges:
                    out_edges_G.add_edge(source, target, weight=G[source][target]['weight'])
                
                self.network_weighted_plot(out_edges_G, 30, 50, net_layout = 'random', directed_edges = False, 
                                           plot_title = str(graph_base_title) + ' - h. cent. node: ' + str(node_higher_degree_cent) + ' (out)')
                
                source = 'pyrolysis'
                target = 'storage'
                #heaviest_path = max([ path for path in nx.all_simple_paths(G, source, target)], key=lambda source, target: G[source][target]['weight'])
                print([ path for path in nx.all_simple_paths(G, source, target)])
                
                #calculando a assortativity
                #print(nx.average_degree_connectivity(G_directed))'''

            elif nodes_to_analyze is None and graph_type.lower() == 'graph':

                print('* Network analysis *')
                print('N nodes: ', nx.number_of_nodes(G))
                print('N edges: ', nx.number_of_edges(G))
                #print('N non edges:', len(list(nx.non_edges(G)))) #o custo da memória é grande para essa operação
                print('Density:', round(nx.density(G), 2))                


            elif nodes_to_analyze is not None and graph_type.lower() == 'digraph':
                
                #varrendo os nodes inseridos
                for node in nodes_to_analyze:
                    
                    #determinando os out_edges
                    out_edges = [edge for edge in G.edges() if edge[0] == node.lower()]
                    out_edges_G = nx.Graph()
                    for source, target in out_edges:
                        out_edges_G.add_edge(source, target, weight=G[source][target]['weight'])
                    ...
                    #determinando os in_edges
                    in_edges = [edge for edge in G.edges() if edge[1] == node.lower()]
                    in_edges_G = nx.Graph()
                    for source, target in in_edges:
                        in_edges_G.add_edge(source, target, weight=G[source][target]['weight'])
    ...
    def network_weighted_plot(self, graph, min_circle_size, max_circle_size, net_layout = 'random', nodes_categories = None, directed_edges = False, plot_title = ''):
        ...
        p = figure(toolbar_location=None, 
                   title=plot_title,                   
                   match_aspect=False,
                   x_range=Range1d(xmin-((xmax-xmin)*0.2), xmax+((xmax-xmin)*0.3)), y_range=Range1d(ymin-((ymax-ymin)*0.2), ymax+((ymax-ymin)*0.3)),
