...
class results(object):
    ...
    def plot_1column_network_digraph(self, DF_column = '', max_circle_size = 100, min_circle_size = 10, min_occurrences = 0, graph_base_title = '',
                                   start_end_nodes_to_analyze = None, path_nodes_cutoff = None, min_node_degree = 0, plot_graphs = False, 
                                   print_unique_vals_in_DF = False):
        ...
        for file in filtered_series.index.levels[0]:
            ...
            try:
                column_vals = filtered_series.loc[ (file, ) , ].values
            except KeyError:
                continue
        ...
        self.network_analysis_graph_plot(concat_network_relat, max_circle_size = max_circle_size, min_circle_size = min_circle_size,
                                         ...
                                         path_nodes_cutoff = path_nodes_cutoff, min_node_degree = min_node_degree, 
                                         plot_graphs = plot_graphs, graph_base_title = graph_base_title,
                                         n_articles = n_articles)    
                
        #passando para array para plotar ocorrências de termos
        self.network_report_dic['cat_occurrences'] = np.array(list( zip( *np.unique(filtered_series_without_duplicates.values.reshape(-1), return_counts=True) ) ))
    ...
    def plot_multicolumn_network_graph(self, DF_columns = ['0', '1', '2'], max_circle_size = 100, min_circle_size = 10, min_occurrences = 10,
                                       start_end_nodes_to_analyze = [], path_nodes_cutoff = None, min_node_degree = 0, plot_graphs = False, 
                                       graph_base_title = ''):
    ...
    def network_analysis_graph_plot(self, concat_network_relat, nodes_categories = None, 
                                    ...
                                    start_end_nodes_to_analyze=[], path_nodes_cutoff = None, plot_graphs = False,
                                    min_node_degree = 0, n_articles = None):
        ...
        for graph_type in ['digraph']: #a análise do graph está desativada
            try:
                print(f'\nAnalisando o {graph_type}...')
                concat_network_relat[graph_type]
                
                #definindo o dic
                self.network_report_dic[graph_type] = {}
                ...
                self.network_report_dic['digraph']['n_nodes'] = nx.number_of_nodes(G)
                self.network_report_dic['digraph']['n_edges'] = nx.number_of_edges(G)
                self.network_report_dic['digraph']['density'] = round(nx.density(G), 4)
                ...
                if graph_type == 'graph':
                    ...
                    self.network_report_dic['graph']['degree_nodes'] = sorted(G.degree(), key = lambda t: t[1] , reverse=True)
                    ...
                    self.network_report_dic['digraph']['in_degree_nodes'] = np.array( in_degrees )
                    self.network_report_dic['digraph']['out_degree_nodes'] = np.array( out_degrees )                    
                    
                    '''
                    #plotando os grafos
                    if plot_graphs is True:
                        node_higher_in_degree = in_degrees[0][0]
                        node_higher_out_degree = out_degrees[0][0]
                        #determinando os out_neighbors para plotar
                        out_edges_G = get_neighbor_graph_from_digraph(node_name = node_higher_out_degree, graph = G, mode= 'out')
                        #determinando os in_neighbors para plotar
                        in_edges_G = get_neighbor_graph_from_digraph(node_name = node_higher_in_degree, graph = G, mode= 'in')
                        self.network_weighted_plot(out_edges_G, 30, 60, net_layout = 'random', directed_edges = False, 
                                                   plot_title = str(graph_type) + ' - ' + str(graph_base_title) + ' - node: ' + str(node_higher_in_degree) + ' (out) [ articles = ' + str(n_articles) + ']')
                        self.network_weighted_plot(in_edges_G, 30, 60, net_layout = 'random', directed_edges = False, 
                                                   plot_title = str(graph_type) + ' - ' + str(graph_base_title) + ' - node: ' + str(node_higher_out_degree) + ' (in) [ articles = ' + str(n_articles) + ']')'''
                ...
                #análise de paths
                if len(start_end_nodes_to_analyze) == 2 and graph_type == 'digraph':
                    ...
                    #self.network_report_dic['digraph']['path'] = {} 
                    #self.network_report_dic['digraph']['path']['heaviest_weight'] = heaviest_weight
                    #self.network_report_dic['digraph']['path']['heaviest_path'] = heaviest_path
        
                    #encontrando o heaviest path para o max clique
                    find_heaviest_path_from_graph(G, start=start_node, end=None, min_edge_weight=0.1, network_type = 'directed', network_name = 'Full network (directed)')            
...
def find_heaviest_path_from_graph(G, start = None, end=None, min_edge_weight=None, network_type='directed', network_name = ''):
    ...
    def search_out_nodes(G, in_node, end_node=None, path = [], max_weight=0, heaviest_path=[], cum_path_weights=[], min_edge_weight = None, close_path = False, counter = 0):
    ...
    if start is not None:
        ...
        _, _, heaviest_path_weight, all_heaviest_path, close_path = search_out_nodes(G, start, end_node = end, min_edge_weight = min_edge_weight, path = [start], max_weight=0, heaviest_path=[], cum_path_weights=[])    
    ...
    #percorrendo todos os possíveis caminhos        
        ...
        for in_node in G.nodes():
            ...
            print('Network: ', network_name)
            print('Start node: ', start)
            _, _, max_weight, heaviest_path, close_path = search_out_nodes(G, in_node, end_node = end, min_edge_weight = min_edge_weight, path = [in_node], max_weight=0, heaviest_path=[], cum_path_weights=[])
