        ...
        for graph_type in ('graph', 'digraph'): #a análise do graph está desativada
            try:
                ...
                if len(start_end_nodes_to_analyze) == 1 and graph_type == 'digraph':
                    ...
                    heaviest_path, heaviest_weight = find_heaviest_path_from_graph(G, start=start_end_nodes_to_analyze[0], min_edge_weight=min_edge_weight, network_type = 'directed', network_name = 'Full network (directed)')
...
class groups_results(object):
    ...
    def __init__(self, *results_dic):
        ...
        for result_type in self.results_types:
            all_vals[result_type] = []

        #encontrandos as categorias únicas para todos os grupos
        for dic in results_dic:
            for result_type in self.results_types:
                if result_type[ : len('unique')] != 'unique':
                    all_vals[result_type] = all_vals[result_type] + [val for val in dic[result_type].keys() if val not in all_vals[result_type]]
        ...
        for dic in results_dic:   
            ...
            for result_type in self.results_types:
                ...
                #resultados de outros tipos
                    ...
                    for cat in all_vals[result_type]:
                        try:
    ...
    def bar_plot(self, maximum_values_to_plot = 20):
        ...
        for group_name, result_type, color in bar_plot_sets:
            print(group_name, result_type)
            if result_type[ : len('unique') ] == 'unique':
                print('entrando unique')
                #encontrando o array para os cat, vals
                results_array = np.array( self.groups_dic[group_name][result_type] )
                
                categories = results_array[:, 0]
                values = results_array[:, 1].astype('float32') / results_array[:, 1].astype('float32').max()                    
                ...
                #encontrando o array para os cat, vals
                results_array = np.array([ [cat , val] for cat, val in self.groups_dic[group_name][result_type] if cat in cats_to_plot])
            ...
            #plotando o gráfico da esquerda
            bar_plots[group_name][result_type] = figure(x_range=categories,
                                                        toolbar_location=None, tools="",
                                                        plot_width=1000, plot_height=1000,
                                                        title = group_name + ': Norm. ' + graph_title[result_type])
            
            bar_plots[group_name][result_type].vbar(categories, 
                                                    top=values, 
                                                    color = color,
                                                    width=0.9)
            
            bar_plots[group_name][result_type].xaxis.axis_label = graph_title[result_type]
            bar_plots[group_name][result_type].xaxis.axis_label_text_font_size = '31pt'
            ...
            bar_plots[group_name][result_type].ygrid.grid_line_color = 'black'
            ...
            bar_plots[group_name][result_type].title.text_font_size = '31pt'
            bar_plots[group_name][result_type].outline_line_color = "black"
            bar_plots[group_name][result_type].outline_line_width = 1
            bar_plots[group_name][result_type].axis.major_label_text_font_size = '31pt'
