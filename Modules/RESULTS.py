...
class results(object):
    ...
    def plot_1column_network_graph(self, DF_column = '', max_circle_size = 100, min_circle_size = 10, min_occurrences = 0, graph_base_title = '',
                                   graph_type = 'graph', analyze = False, nodes_to_analyze = None, plot_full_network = False, print_unique_vals_in_DF = False):
        ...
        filtered_series = self.clear_duplicates_in_columns(filtered_series, column_names=[DF_column], DF_indexes_names = ['doc', 'counter'])
        ...
        concat_network_relat['weight'] = concat_network_relat['weight'].values / concat_network_relat['weight'].values.max()
        
        self.network_analysis_graph_plot(concat_network_relat, max_circle_size = max_circle_size, min_circle_size = min_circle_size,
                                         graph_type = graph_type, analyze = analyze, nodes_to_analyze=nodes_to_analyze, 
                                         plot_full_network = plot_full_network,
                                         graph_base_title = graph_base_title)



    def plot_multicolumn_network_graph(self, DF_columns = ['0', '1', '2'], max_circle_size = 100, min_circle_size = 10, min_occurrences = 10,
                                       graph_type = 'graph', analyze = False, nodes_to_analyze = None, plot_full_network = False, graph_base_title = ''):
    ...
    def network_analysis_graph_plot(self, concat_network_relat, nodes_categories = None, 
                                    ...
                                    graph_type = 'graph', analyze = False, nodes_to_analyze=None, plot_full_network = False):
        ...
        edges_weights_DF['weight'] = concat_network_relat['weight'].values.astype('float32')
        ...
        if analyze is True:
            ...
            #gerando o gráfico undirected
            G_undirected = G
            if graph_type.lower() == 'digraph':
                G_undirected = G.to_undirected()


            '''#cliques
            all_cliques = list(nx.find_cliques(G_undirected))
            max_clique_node_names = sorted(all_cliques, reverse=True, key=lambda t: len(t))[0]
            max_clique_subraph = get_subgraph(nodes_names = max_clique_node_names, graph = G_undirected)
            
            self.network_weighted_plot(max_clique_subraph, 30, 60, net_layout = 'random', directed_edges = False, 
                                       plot_title = str(graph_base_title) + ': largest clique')
            
            #modularity community
            communities = community.girvan_newman(G_undirected)'''

            if nodes_to_analyze is not None and graph_type.lower() == 'digraph':
                
                #varrendo os nodes inseridos
                for node in nodes_to_analyze:
                    
                    #determinando os out_edges
                    out_edges_G = get_neighbor_graph(node_name = node, graph = G, mode= 'out')                    
                    #self.network_weighted_plot(out_edges_G, 30, 60, net_layout = 'random', directed_edges = False, 
                    #                           plot_title = str(graph_base_title) + ' - node: ' + str(node) + ' (out)')
                    
                    #determinando os in_edges
                    in_edges_G = get_neighbor_graph(node_name = node, graph = G, mode= 'in')
                    #self.network_weighted_plot(in_edges_G, 30, 60, net_layout = 'random', directed_edges = False, 
                    #                           plot_title = str(graph_base_title) + ' - node: ' + str(node) + ' (in)')

                    #encontrando o heaviest path para o max clique
                    find_heaviest_path(G, start=node, network_type = 'directed', network_name = 'Full network (directed)')
        
        #plotando a rede
        if plot_full_network is True:
    ...
    def network_weighted_plot(self, graph, min_circle_size, max_circle_size, net_layout = 'random', nodes_categories = None, directed_edges = False, plot_title = ''):
        ...
        if nodes_categories is not None:
            palette_for_cats = ["#f25f5c", "#247ba0", "#70c1b3", "#50514f"]
            #definindo um dicionário de número para categoria
            cats_number_dic = dict( zip( list(set(nodes_categories.values())) , list(range(len(set(nodes_categories.values())))) ))
            for node in nodes_categories.keys():
                nodes_cat = nodes_categories[node]
                nodes_color[node] = palette_for_cats[ cats_number_dic[nodes_cat] ]
        
        #definindo uma cor para cada node
        else:
            palette_nodes = turbo(len(G.nodes()))
            #definindo um dicionário de número para node
            cats_number_dic = dict( zip( list(set(G.nodes().keys())) , list(range(len(set(G.nodes().keys())))) ))
            for node in G.nodes():
    ...
    def group_num_boxplot(self, grouped_DF = None, input1 = '', input2 = '', axes_label1 = '', axes_label2 = '', ymin = None, ymax = None, 
        ...
        p = figure(toolbar_location=None,
                   match_aspect=False,
                   title = 'min occurences: ' + str(min_occurrences,),
                   x_range=cats,                  
                   background_fill_color='white', 
                   plot_width=plot_width, plot_height=plot_height,
                   min_border=200)
    ...
    def pca_scatter_plot(self, DF = None, pca_model = None, loadings_label = ['', ''], 
        ...
        if find_clusters is True:
            ...
            DF_labeled = DF_labeled.drop_duplicates()   ###################################################### checar
        ...
        if find_clusters is True:
            ...
            for xy_val_label in xy_vals_labeled:
                p.circle(xy_val_label[0][0], xy_val_label[0][1], color=self.palette_colors1[ xy_val_label[1] ] , size=25, alpha=0.4)
            ...
            p.circle(x, y, color="gray", size=25, alpha=0.4)
    ...
    def num_num_scatterplot(self, DF = None, axes_labels = ['', ''], input1 = '', input2 = '', 
        ...
        if find_clusters is True:
            ...
            #adicionando os pontos
            for xy_val_label in xy_vals_labeled:
                p.circle(xy_val_label[0][0], xy_val_label[0][1], color=self.palette_colors1[ xy_val_label[1] ], size=25, alpha=0.6)
            #adicionando os centroides
            for i in range(len(c_centers)):
                p.circle(c_centers[i][0], c_centers[i][1], line_color='black', line_width=3, fill_color=self.palette_colors1[i] , size=40, alpha=0.8)
            #adicionando os textos
            ...
            p.circle(x, y, color="blue", size=25, alpha=0.6)
        
        if regression is True:
            #fazendo a regressão linear
            b0, b1, r_sq = lin_regression(x,y)
            p.line([xmin, xmax],[b0 + ymin*b1, b0 + ymax*b1], line_width=10, color='black', line_alpha=0.6, line_dash='dashed')
    ...
    def group_num_num_scatterplot(self, DF = None, grouped = None, axes_labels = ['', ''], input1 = '', input2 = '', 
        ...
        for group in grouped.groups:
            ...
            p.circle(x, y, color=self.palette_colors1[counter], size=30, alpha=0.5, line_color='black', line_width=1)
...
def set_bokeh_horizontal_hist(main_plot):
    ...
    ph.yaxis.major_label_text_font_size = '30pt'
    ph.yaxis.axis_label_text_font_size = '30pt'
...
def set_bokeh_vertical_hist(main_plot):
    ...
    pv.xaxis.major_label_text_font_size = '30pt'
    pv.xaxis.axis_label_text_font_size = '30pt'
...
def find_heaviest_path(G, start = None, network_type='directed', network_name = ''):
    
    #import time
    #print(G.nodes())
    #print('\n')
    
    def search_out_nodes(G, in_node, path = [], max_weight=0, heaviest_path=[], cum_path_weights=[]):
        ...
        for out_node in node_list:
            ...
            if out_node not in path:
                ...
                #print('in: ', in_node, ' - out: ', out_node, ' - path weight: ', path_weight, ' - cum path weight: ', sum(cum_path_weights), ' - max weight: ', max_weight)
                #print('current path: ', path)
    
                #recurssão
                new_cum_path_weights, new_path, max_weight, heaviest_path = search_out_nodes(G, out_node, path=path, max_weight=max_weight, heaviest_path=heaviest_path, cum_path_weights=cum_path_weights)
                #print('checking:', sum(new_cum_path_weights), max_weight)
                if new_cum_path_weights is not None and sum(new_cum_path_weights) > max_weight:
                    ...
                    #print('> current path: ', path)
                    #print('> cum path weight: ', sum(cum_path_weights))
                    
                path = path[ : -1]
                cum_path_weights = cum_path_weights[ : -1]
    
            #time.sleep(1)
        
    
        #print('Nenhum out_node encontrado!')
        #print('last path: ', path)
        #print('cum path weight: ', sum(cum_path_weights), ' - max_weight: ', max_weight)
        return cum_path_weights, path, max_weight, heaviest_path
    ...
    if start is not None:
        ...
        _, _, heaviest_path_weight, all_heaviest_path = search_out_nodes(G, start, path = [start], max_weight=0, heaviest_path=[], cum_path_weights=[])    
        print('Heaviest path: ', all_heaviest_path)
        print('Heaviest path weight: ', heaviest_path_weight)

    #percorrendo todos os possíveis caminhos        
    else:
        print('\nFinding heaviest path...')
        ...
        for in_node in G.nodes():
            ...
            print('Network: ', network_name)
            print('Start node: ', start)
            _, _, max_weight, heaviest_path = search_out_nodes(G, in_node, path = [in_node], max_weight=0, heaviest_path=[], cum_path_weights=[])
    
            if max_weight > heaviest_path_weight:
                all_heaviest_path = heaviest_path
                heaviest_path_weight = max_weight
                print('Heaviest path atualizado...')
                print('Heaviest path: ', all_heaviest_path)
                print('Heaviest path weight: ', heaviest_path_weight)



def get_neighbor_graph(node_name = '', graph = None, mode= 'in'):
