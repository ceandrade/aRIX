...
class results(object):
    ...
    def __init__(self, results_DF_input_name = 'DF_FULL'):
        ...
        if not os.path.exists(self.diretorio + '/Settings/Index_lists.json'):
            save_dic_to_json(self.diretorio + '/Settings/Index_lists.json', dict())
    ...
    def plot_num_num_correlation(self, DF_columns_with_num_vals = ['a', 'b'], axes_labels = ['d', 'e'], x_quantiles = [0, 1], y_quantiles = [0, 1],
    ...
    def cat_cat_gridplot(self, DF = None, DF_x_column = '', DF_y_column = '', DF_val_column = '', 
        ...
        p = figure(toolbar_location=None,
                   title='articles used = ' + str(proc_vals),
                   background_fill_color = background_fill_color,
                   min_border = 200,
    ...
    def pca_scatter_plot(self, DF = None, pca_model = None, loadings_label = ['', ''], 
        ...
        #plotando os scores
        if find_clusters is True:
            colors = ['#4EACC5', '#FF9C34', '#4E9A06', '#BE2929', ]        
            cluster_labels = [cluster_preffix + str(i) for i in range(1, n_clusters+1)]
            #adicionando os pontos
            for xy_val_label in xy_vals_labeled:
                p.circle(xy_val_label[0][0], xy_val_label[0][1], color=colors[ xy_val_label[1] ] , size=16, alpha=0.4)            
            #adicionando os centroides
            for i in range(len(c_centers)):
                p.circle(c_centers[i][0], c_centers[i][1], line_color='black', line_width=3, fill_color=colors[i] , size=26, alpha=0.8)
            #adicionando os textos
        ...
        if find_clusters is True and export_groups_to_csv is True:
            from FUNCTIONS import load_dic_from_json
            from FUNCTIONS import save_dic_to_json
            index_lists_dic = load_dic_from_json(self.diretorio + '/Settings/Index_lists.json')
            ...
            save_dic_to_json(self.diretorio + '/Settings/Index_lists.json', index_lists_dic)
            print('Salvando a index_list dos grupos em ~/Settings/Index_lists.json ...')        
    ...
    def num_num_scatterplot(self, DF = None, axes_labels = ['', ''], input1 = '', input2 = '', 
        ...
        #salvando os grupos
        if find_clusters is True and export_groups_to_csv is True:
            print(f'Salvando os grupos em ~/Outputs/Plots/P{self.last_fig_filename_index}.csv ...')
            DF_labeled.to_csv(self.diretorio + f'/Outputs/DataFrames/Groups/groups_P{self.last_fig_filename_index}.csv')
