    ...
    def extract_numerical_parameters(self, text, text_index, parameter, extract_mode = 'first'):
        ...
        def extract_PU_and_numbers_from_text(match_text):
            ...
            for raw_num in num_list_extracted:
                try:
                    if round(float(raw_num), 9) != 0.000000000:
        ...
        if extract_mode.lower() == 'first':            
            ...
            match_prefixes_to_find = None
            prefixes_check = False
            ...
            #encontrando os numéricos com as unidades físicas que são desejadas
            match_numbers_to_find = re.search( parameters_patterns['PU_to_find_regex'] , text)
            
            #caso o match de numéricos tenha sido encontrado
            if match_numbers_to_find:
                ...
                #checando se há termos que podem ou não podem ser encontrados antes dos números e PUs
                match_prefixes_to_find = re.search(parameters_patterns['pattern_variation_parameter'],
                                                   text[ : matches_end ] )
                ...
                if (match_prefixes_to_find is not None) and parameters_patterns['find_variation_parameter'] is True:
                    print('prefix_text to search: ', text[ : matches_end ])
                ...
                #encontrando os numéricos com as unidades físicas que não são desejadas
            ...
            if ( match_numbers_to_find is not None ) and ( match_numbers_further_in_sent is None ) and (match_numbers_not_to_find is None) and (prefixes_check is True):
                #texto encontrado
        ...
        #caso sejam coletados todos os findings na sentença
            ...
            cumulative_cut_numbers_list = []
            cumulative_cut_parameters_list = []
            ...
            while True:
                ...
                if parameters_patterns['parameters_pattern_to_find_in_sent'] is not None:
                    ...
                    if match_parameters_to_find:
                        ...
                        cumulative_cut_parameters += match_parameters_to_find.span()[1]
                        cumulative_cut_parameters_list.append(cumulative_cut_parameters)
                        #extraindo os parâmetros da sentença
                        parameters_extracted_from_sent = extract_parameters_from_text( match_parameters_to_find.captures()[0] )
                ...
                #caso haja parametros extraidos do texto
                if parameters_extracted_from_sent is not None:
                    n_numbers_to_extract = len(parameters_extracted_from_sent) * 2
                ...
                if match_numbers_to_find:
                    ...
                    print('match_numbers_to_find: ', match_numbers_to_find)
                    matches_start, matches_end = match_numbers_to_find.span()
                    cumulative_cut = matches_end + cumulative_cut_parameters

                    #checando se há termos que podem ou não podem ser encontrados antes dos números e PUs (prefixes)
                    prefix_text = text[ cumulative_cut_numbers : cumulative_cut_numbers + cumulative_cut ]
                    match_prefixes_to_find = re.search(parameters_patterns['pattern_variation_parameter'],
                                                       prefix_text)

                    #adicionando o corte cumulativo para os números
                    cumulative_cut_numbers += cumulative_cut
                    cumulative_cut_numbers_list.append(cumulative_cut_numbers)
                    ...
                    if parameters_patterns['PU_not_to_find_regex'] is not None:
                        ...
                        match_numbers_not_to_find = re.search(parameters_patterns['PU_not_to_find_regex'] , 
                                                              text[ matches_start + cumulative_cut_parameters : matches_end + cumulative_cut_parameters + 7 ] )
                        if match_numbers_not_to_find:                            
                    ...
                    #caso o número de parâmetros extraídos seja 1, 2 ou multiplo de 2
                    if len(num_list_extracted_raw_temp) <= n_numbers_to_extract:                    
                        ...
                        #print('all 4. cumulative_cut_numbers_list: ', cumulative_cut_numbers_list)
                        #print('all 5. cumulative_cut_parameters_list: ', cumulative_cut_parameters_list)
                        #time.sleep(2)
                        continue
            ...
            if None in (factor_to_multiply , factor_to_add , SI_units):            
                ...
                for num in num_list_extracted_raw:
                    if '&' in num:
                        ...
                        first_number = round( float(numbers_interval[0]) * factor_to_multiply + factor_to_add , 9)
                        last_number = round( float(numbers_interval[1]) * factor_to_multiply + factor_to_add , 9)
