...
class WV(object):
    ...
    def split_TFIDF_to_SPARSE(self, batch_size = 50000):
        ...
        if not os.path.exists(self.diretorio + '/Outputs/TFIDF/SPARSE'):
            os.makedirs(self.diretorio + '/Outputs/TFIDF/SPARSE')
        ...
        try:
            all_sparse_files = get_filenames_from_folder(self.diretorio + '/Outputs/TFIDF/SPARSE', file_type = 'npz')
        ...
        for index in batch_indexes:            
            ...
            print(f'Salvando a sparse ~/Outputs/TFIDF/SPARSE/sparse_csr_rNgram_{self.replace_Ngrams}_{counter_npz_to_save}.npz')
            sparse.save_npz(self.diretorio + f'/Outputs/TFIDF/SPARSE/sparse_csr_rNgram_{self.replace_Ngrams}_{counter_npz_to_save}.npz', sm, compressed=True)
            h5_M.close()        
            counter_npz += 1
    ...
    def get_LSA_wv(self, n_dimensions = 300):
        ...
        file_list = os.listdir(self.diretorio + '/Outputs/TFIDF/SPARSE') #lista de arquivos
        NPZ_documents = []
        for filename in file_list:
        ...
        if (self.mode == 'truncated_svd'):
            ...
            for filename in [filename for filename in NPZ_documents if (f'_rNgram_{self.replace_Ngrams}' in filename)]:
                ...
                m = sparse.load_npz(self.diretorio + '/Outputs/TFIDF/SPARSE/' + filename)
    ...
    def get_W2Vec(self, wv_dim = 300, n_epochs = 5, batch_size = 50, pdf_batch_size = 10):
        ...
        for file_tag in proc_documents[ file_index : ]:    
            ...
            if pdf_batch_counter % pdf_batch_size == 0 or file_tag == proc_documents[-1]:
                ...
                #plotando os treinamentos
                self.plot_2DWV( words=['carbon', 'chicken', 'coconut', 'shell', 'cattle', 'manure','sugarcane', 'bagasse', 
                                       'graphene', 'oxide',  'go', 'metals'],
    ...
    def find_terms_sem_similarity(self, n_similar_terms = 5, minimum_app_count = 1):
        ...
        for filename in filtered_filenames:
            ...
            for token in n1gram_sim_terms_DF.index:
                if n1gram_sim_terms_DF.loc[token, 'Sem_App_Counter'] >= minimum_app_count:
                    for neighbor in n1gram_sim_terms_DF.index:
                        if n1gram_sim_terms_DF.loc[neighbor, 'Sem_App_Counter'] >= minimum_app_count:
                            if token != neighbor:
    ...
    def combine_topic_vectors_by_sem_similarity(self, n_topics = 10, min_sem_app_count = 4):
        ...
        filtered_filenames = [filename for filename in filenames if filename[ : 6 ] == 'n1gram']
                    
        for filename in filtered_filenames:
            #carregando a lista de termos a serem pesquisados
            n1gram_DF = pd.read_csv(self.diretorio + f'/Outputs/nGrams/Semantic/{filename}.csv', index_col=0)
            words_to_find_similatiry = [index for index in n1gram_DF.index if n1gram_DF.loc[index, 'Sem_App_Counter'] >= min_sem_app_count]
            ...
            if len(words_to_find_similatiry) > 1:
                ...
                for term in words_to_find_similatiry:
                    ...
                    larg_topic_val_index = get_largest_topic_value_indexes(topic_vec, n_topics = n_topics, get_only_positive=True)
                ...
                print(f'Os termos em "{filename}" com valores "Sem_App_Counter" abaixo do threshold ("min_sem_app_count" {min_sem_app_count})')
                time.sleep(3)
