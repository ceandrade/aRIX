...
def main():
    ...
    classPDFs = classifier(master_terms = ['hydrochar', 'biochar'], save_to_GoogleDrive = False, diretorio = diretorio)
    ...
    t_objects.get_text_objects(save_TXT = False, save_JSON = True, filter_by_TF = True, min_tokens_to_break_sent = 2, min_tokens_per_sent = 2)
    ...
    t_objects.find_text_sections(sections_names=[r'[0-9\s\.]*(Experimental|Materials and [Mm]ethods)\s*[0-9]\.', r'[0-9\s\.]*Results and [Dd]iscussion\s*[0-9]\.'])
    t_objects.get_Ngrams_appearance(min_sent_tokens = 5)
    #t_objects.filter_Ngrams(N = 2, threshold_2gram = 300, threshold_3gram = 200, n2gram_mim_score_allowed = 1e-7, n3gram_mim_score_allowed = 1e-16)
    t_objects.find_IDF(min_token_appereance = 50)
    t_objects.set_log_for_TFIDF(n_process = 1)
    t_objects.find_TFIDF(process_number = 1)
    ...
    wv.find_terms_sem_similarity(n_similar_terms_to_overlap = 30, minimum_app_count_to_put_into_2grams = 2)


    #determinando a sobreposição de tópicos usando os termos com similaridade semântica
    wv = WV(wv_model = 'svd', replace_Ngrams = False, diretorio = diretorio) #(w2vec or gensim or svd))
    wv.set_svd_parameters(mode = 'truncated_svd', filter_stopwords = False)
    wv.combine_topic_vectors_by_sem_similarity(n_largest_topic_vals = 30, min_sem_app_count_to_get_topic = 4)
